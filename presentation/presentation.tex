...
%
\documentclass[12pt,ignorenonframetext,handout,]{beamer}
...
\hypertarget{objectifs-et-organisation}{%
...
Apprendre à perfectionner son utilisation de R : acquérir des points de
repères, des réflexes, des méthodes de travail.
...
\bigskip \pause \(\Rightarrow\) Mettre en place des
\intertitre{fondations solides} pour permettre votre progression dans
...
\protect\hypertarget{savoir-utiliser-laide-du-logiciel}{}
...
\intertitre{Remarque} Pour afficher l’aide sur une fonction d’un
...
\protect\hypertarget{chercher-de-laide-en-ligne}{}
...
Quand une question semble ne pas avoir été déjà posée, ne pas hésiter à
la poser soi-même, en joignant alors un \textbf{exemple reproductible}
(\emph{minimal working example} ou MWE).
...
\protect\hypertarget{se-reperer-dans-les-cran}{}
...
\href{https://cran.r-project.org/web/views/ModelDeployment.html}{ModelDeployment},
...
\protect\hypertarget{installer-un-package-manuellement}{}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Note : Le fichier haven._1.1.0.zip est situé }
\CommentTok{# dans le répertoire de travail}
\KeywordTok{install.packages}\NormalTok{(}
  \StringTok{"haven_1.1.0.zip"}\NormalTok{, }\DataTypeTok{repos =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{type =} \StringTok{"binaries"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\protect\hypertarget{installer-des-packages-depuis-github}{}
...
\intertitre{Exemple} \url{https://github.com/tidyverse/haven}
...
\protect\hypertarget{quest-ce-que-travailler-efficacement-avec-r}{}
...
\pause \intertitre{Ce n'est pas toujours vrai} : on perd souvent
beaucoup de temps à (ré)inventer une méthode peu efficace quand une
beaucoup plus simple et rapide existe déjà.
...
\protect\hypertarget{construire-un-exemple-reproductible-mwe}{}
...
\begin{frame}{Plan de la partie}
\protect\hypertarget{plan-de-la-partie-1}{}
...
\hypertarget{de-limportance-des-fonctions-dans-r}{%
...
\fg{}}
\protect\hypertarget{tout-ce-qui-agit-est-un-appel-de-fonction}{}
...
\protect\hypertarget{definir-une-fonction-dans-r}{}
...
\protect\hypertarget{valeurs-par-defaut-des-parametres}{}
...
\pause \intertitre{Remarque} Ceci est la conséquence de la \emph{lazy
...
\texttt{apply()}}
\protect\hypertarget{appliquer-sur-chaque-independamment-apply}{}
...
\begin{frame}[fragile]{\large Appliquer sur chaque indépendamment :
\texttt{lapply()}}
\protect\hypertarget{appliquer-sur-chaque-independamment-lapply}{}
...
\intertitre{Exemple d'utilisation} Appliquer une fonction à toutes les
...
\begin{frame}[fragile]{\large Appliquer sur chaque indépendamment :
\texttt{sapply()}}
\protect\hypertarget{appliquer-sur-chaque-independamment-sapply}{}
...
\pause \intertitre{Exemple d'utilisation} Calcul de statistiques sur
toutes les variables d’une table.

\end{frame}

\begin{frame}[fragile]{\large Définir une fonction à la volée dans
\texttt{*apply()}}
\protect\hypertarget{definir-une-fonction-a-la-volee-dans-apply}{}
...
\protect\hypertarget{utiliser-les-boucles-avec-parcimonie-1}{}
...
\protect\hypertarget{simplifier-les-donnees-le-type-factor}{}
...
\protect\hypertarget{utiliser-les-noms-a-bon-escient-1}{}
...
\intertitre{Exemple} On cherche à extraire les observations d’une table
...
\protect\hypertarget{a-propos-des-matrices-2}{}
...
\NormalTok{  ##   dense 1289.568 1306.541 1365.1855 1326.277 1396.538}
\NormalTok{  ##  sparse   69.868   76.775  104.6503  109.340  114.076}
\NormalTok{  ##       max neval}
\NormalTok{  ##  1687.056   100}
\NormalTok{  ##   230.520   100}
\end{Highlighting}
\end{Shaded}

\end{frame}

\hypertarget{dplyr-une-grammaire-du-traitement-des-donnees}{%
...
\protect\hypertarget{philosophie-de-dplyr}{}
...
\intertitre{Pour en savoir plus} De nombreuses vignettes très
...
\protect\hypertarget{donnees-dexemple-table-flights-de-nycflights13}{}
...
\texttt{\%\textgreater{}\%}}
\protect\hypertarget{enchainer-des-operations-avec}{}
...
\protect\hypertarget{fusionner-des-tables-avec-_join}{}
...
\intertitre{Pour en savoir plus} Une
...
\protect\hypertarget{comparaison-de-base-r-et-de-dplyr}{}
...
\NormalTok{)}
\NormalTok{  ## Note: no visible binding for global variable 'x' }
\NormalTok{  ## Note: no visible binding for global variable 'x' }
\NormalTok{  ## Note: no visible binding for global variable 'x' }
\NormalTok{  ## Note: no visible binding for global variable 'x' }
\NormalTok{  ## Note: no visible binding for global variable 'x' }
...
\NormalTok{  ## Unit: milliseconds}
\NormalTok{  ##   expr      min       lq     mean   median       uq}
\NormalTok{  ##   base 43.73911 45.97799 53.84660 54.30748 60.82400}
\NormalTok{  ##  dplyr 61.99594 62.22211 67.85551 62.54855 67.66261}
\NormalTok{  ##       max neval}
\NormalTok{  ##  63.56773    10}
\NormalTok{  ##  92.79042    10}
\end{Highlighting}
\end{Shaded}
...
\protect\hypertarget{selectionner-des-observations-avec-i}{}
...
\begin{itemize}
\item
  \pause il n’y a pas à répéter le nom du \texttt{data.frame} dans
  \texttt{{[}};
\item
  \pause il est possible d’indexer un \texttt{data.table} par une ou
  plusieurs \og clés \fg{} permettant une recherche souvent plus rapide.
\end{itemize}
...
\NormalTok{  ## Unit: milliseconds}
\NormalTok{  ##  expr      min       lq     mean   median       uq}
\NormalTok{  ##  base 48.28950 55.54021 68.42902 62.12264 65.70280}
\NormalTok{  ##   dt1 13.61763 15.67296 22.64325 18.28742 26.11897}
\NormalTok{  ##   dt2 12.76936 15.81703 22.46256 17.85401 25.05992}
\NormalTok{  ##       max neval}
\NormalTok{  ##  273.7490   100}
\NormalTok{  ##  229.3413   100}
\NormalTok{  ##  203.6016   100}
\end{Highlighting}
\end{Shaded}
...
\protect\hypertarget{calculer-des-statistiques-avec-j}{}
...
\texttt{keyby}}
\protect\hypertarget{ventiler-des-traitements-avec-by-et-keyby}{}
...
\pause \intertitre{Remarque} Par défaut, \texttt{by} ordonne les
résultats dans l’ordre des groupes dans le \texttt{data.table}.
\texttt{keyby} trie les données selon la variable d’agrégation (comme
\texttt{group\_by} de \texttt{dplyr}).

\end{frame}

\begin{frame}[fragile]{Chaîner les opérations dans un
\texttt{data.table}}
\protect\hypertarget{chainer-les-operations-dans-un-data.table}{}
...
\pause \intertitre{Remarque} Ces chaînages sont possibles avec un
...
\protect\hypertarget{les-limites-du-logiciel}{}
...
\protect\hypertarget{se-connecter-a-des-bases-de-donnees}{}
...
\texttt{dplyr}}
\protect\hypertarget{se-connecter-a-des-bases-de-donnees-avec-dplyr}{}
...
\texttt{parallel} (1)}
\protect\hypertarget{paralleliser-des-traitements-avec-parallel-1}{}
...
\pause Par défaut, R n’expoite qu’un seul c\oe ur : le \emph{package}
...
\tightlist
\item
  \pause Création et paramétrage du \og \textit{cluster} \fg{} de
  c\oe urs à utiliser (chargement des fonctions et \emph{packages}
  nécessaires sur chaque c\oe ur);
\item
  \pause Lancement du traitement parallélisé avec \texttt{parLapply()};
\item
  \pause Arrêt des processus du \emph{cluster} avec
  \texttt{stopCluster()}.
\end{enumerate}
...
(1)}
\protect\hypertarget{rcpp-un-package-r-pour-utiliser-c-1}{}
...
\intertitre{Remarque} Il est également possible de soumettre un fichier
...
\begin{frame}[fragile]{\texttt{Rcpp} : un package R pour utiliser C++
(2)}
\protect\hypertarget{rcpp-un-package-r-pour-utiliser-c-2}{}
...
\protect\hypertarget{r-et-la-realisation-de-graphiques}{}
...
\protect\hypertarget{base-r-ou-ggplot2}{}
...
\texttt{ggplot2}}
\protect\hypertarget{donnees-dexemple-table-mpg-de-ggplot2}{}
...
\hypertarget{realiser-des-graphiques-avec-graphics}{%
...
\includegraphics{presentation_files/figure-beamer/unnamed-chunk-91-1.pdf}
...
\begin{frame}[fragile]{Couleur, forme et taille des objets}
\protect\hypertarget{couleur-forme-et-taille-des-objets-2}{}
...
\includegraphics{presentation_files/figure-beamer/unnamed-chunk-95-1.pdf}

\end{frame}

\begin{frame}[fragile]{Titres, texte et axes}
\protect\hypertarget{titres-texte-et-axes}{}
...
décalage (pour nommer des points par exemple). \pause Il est également
possible de paramétrer les axes :
...
\intertitre{Remarque} Pour produire un graphique sans axe et les
rajouter après, utiliser l’option \texttt{axes\ =\ FALSE} de la fonction
\texttt{plot()}.

\end{frame}

\begin{frame}[fragile]{Combinaison de plusieurs graphiques}
\protect\hypertarget{combinaison-de-plusieurs-graphiques}{}
...
\intertitre{Exemple} Ajout d’une droite de régression au graphique de
...
\includegraphics{presentation_files/figure-beamer/unnamed-chunk-98-1.pdf}
...
\NormalTok{  ## Warning in grid.Call(L_stringMetric, as.graphicsAnnot(x}
\NormalTok{  ## $label)): taille de police inconnue pour le caractère}
\NormalTok{  ## 0x17}
\end{Highlighting}
\end{Shaded}
...
documents}
\protect\hypertarget{principe-de-la-generation-automatique-de-documents}{}
...
documents}
\protect\hypertarget{etapes-de-la-generation-automatique-de-documents}{}
...
\protect\hypertarget{ecrire-du-texte-dans-r-markdown}{}
...
\protect\hypertarget{ecrire-du-code-dans-r-markdown}{}
...
\includegraphics{presentation_files/figure-beamer/unnamed-chunk-148-1.pdf}
...
\protect\hypertarget{integrer-des-tableaux-dans-r-markdown}{}
...
\begin{frame}[fragile]{Intégrer des tableaux dans R Markdown}
\protect\hypertarget{integrer-des-tableaux-dans-r-markdown-1}{}
...
\begin{frame}[fragile]{Intégrer des tableaux dans R Markdown}
\protect\hypertarget{integrer-des-tableaux-dans-r-markdown-2}{}
...
\protect\hypertarget{parametrer-un-document-r-markdown}{}
